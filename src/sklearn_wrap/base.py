...
# TODO: Metadata routing to instance? To nested wrapped estimators instanciated in instanciate()?
# TODO: Improve error messages and use !r only when it makes sense
# TODO: Improve example in docstring
class BaseClassWrapper(BaseEstimator, metaclass=abc.ABCMeta):
    ...
    **params
    ...
    >>>
    >>> # Use it like any sklearn estimator
    >>> estimator = MyEstimator(SimpleRegressor, multiplier=2.0)
    >>> estimator.get_params()
    {'regressor': <class 'SimpleRegressor'>, 'multiplier': 2.0}
    >>> X = np.array([[1, 2], [2, 3], [3, 4]])
    >>> y = np.array([1, 0, 1])
    >>> estimator.fit(X, y).predict(X)
    array([2., 2., 2.])
    >>> estimator.set_params(multiplier=3.0).fit(X, y).predict(X)
    array([3., 3., 3.])
    """
    ...
    def _validate_estimator_class(self, estimator_class: type) -> type:
        ...
        if not issubclass(estimator_class, self.estimator_base_class):
            ...
            raise ValueError(
                f"Invalid {self._estimator_name} class {type(estimator_class).__qualname__!r} for estimator "
                f"{self.__class__.__name__}. Valid estimator class should be derived from "
                f"{base_class_name!r}."
            )
    ...
    def _validate_estimator_params(self, params: dict, *, validate_nested: bool = True):
        ...
        for param_name, param_val in params.items():
            ...
            if param_name not in valid_class_params and not has_var_keyword:
                raise ValueError(f"{param_name} is not a valid parameter for class {self.estimator_class}.")
    ...
    def instantiate(self) -> "BaseClassWrapper":
        ...
        """
        # TODO: Dynamic update of _parameter_constraints?
        self._validate_params()

        for param_name, param_value in self.params.items():
            if param_value == REQUIRED_PARAM_VALUE:
                raise ValueError(f"Class `{self.estimator_class.__name__}` requires parameter {param_name}.")
