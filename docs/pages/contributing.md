# Contributing to Sklearn-Wrap

Thank you for your interest in contributing to Sklearn-Wrap! This document provides guidelines for contributing to the project.
...
Sklearn-Wrap uses pytest with markers to categorize tests into different types:
...
=== "uv run"
    ...
    ```bash
    uv run pytest --cov=sklearn_wrap --cov-report=html
    ```
...
- **API cross-links**: When mentioning sklearn_wrap classes or functions in markdown cells, wrap them in backtick-link syntax pointing to the API page (e.g., `` [`SeasonalNaive`](/pages/api/generated/sklearn_wrap.point.naive.SeasonalNaive/) ``).
...
- [Open an issue on GitHub](https://github.com/stateful-y/sklearn-wrap/issues/new)
- [Start a discussion in the repository](https://github.com/stateful-y/sklearn-wrap/discussions)
