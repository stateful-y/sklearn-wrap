...
3. **gridsearch.py** - Apply to hyperparameter tuning

## Examples

### [first_wrapper.py](../examples/first_wrapper/)

**Creating Your First Sklearn-Compatible Wrapper** (12 minutes)

Start here to understand the fundamental wrapper pattern. This example walks through wrapping a custom polynomial regression algorithm that uses gradient descent. You'll learn the three essential components every wrapper needs: the `_estimator_name` and `_estimator_base_class` attributes, plus the `@_fit_context` decorator. By the end, you'll have a working wrapper that integrates seamlessly with Scikit-Learn's ecosystem. This is the foundation for everything else.
...
**Error Patterns and Parameter Validation** (12 minutes)
...
### [gridsearch.py](../examples/gridsearch/)

**Hyperparameter Tuning with GridSearchCV** (10 minutes)
...
### [xgboost.py](../examples/xgboost/)

**Wrapping XGBoost's Booster API** (15 minutes)
...
**Persistence with Joblib** (10 minutes)
...
**Controlling Nested Estimator Hierarchies** (15 minutes)
...
**Understanding the _fit_context Decorator** (15 minutes)
