...
test:
    uv run pytest tests src/sklearn_wrap --doctest-modules --doctest-continue-on-failure -n auto -v
...
test-cov:
    uv run pytest --cov=sklearn_wrap --cov-report=html --cov-report=term -n auto

# Run docstring examples
test-docstrings:
    uv run pytest --doctest-modules --doctest-continue-on-failure --no-cov src/sklearn_wrap
...
link:
    uvx linkchecker {{ site_dir }}/index.html --no-status --no-warnings --ignore-url '/edit/$'
