    ...
    class KNNClassifier:
        ...
        def _calculate_distances(self, x):
            if self.distance_metric == "euclidean":
                return np.sqrt(((self.X_train_ - x) ** 2).sum(axis=1))
            elif self.distance_metric == "manhattan":
                return np.abs(self.X_train_ - x).sum(axis=1)
            else:
...
def _(GridSearchCV, KNNClassifier, KNNWrapper):
    ...
    return X_train, best_params, best_score, cv_results, test_score, y_train
...
@app.cell
def _(grid, mo):
    mo.md("### GridSearchCV Object")
    grid
    return


@app.cell
def _(best_estimator, mo):
    mo.md("### Best Estimator")
    best_estimator
    return
