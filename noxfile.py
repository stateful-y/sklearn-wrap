...
def test(session: nox.Session) -> None:
    ...
    # Run unit tests directly (no coverage for multi-version tests) with parallel execution
    session.run(
        "pytest",
        "tests",
        "-n",
...
def test_examples(session: nox.Session) -> None:
    ...
    notebooks = sorted(examples_dir.glob("*.py"))

    if not notebooks:
        session.error("No example notebooks found in examples/")

    # Run each notebook as a script (non-interactive execution)
    session.log(f"Running {len(notebooks)} example notebooks...")
    for notebook in notebooks:
        session.log(f"  Testing {notebook.name}")
        session.run("marimo", "run", "--mode", "script", str(notebook), *session.posargs)
